...
picArray.forEach((imgData) => {
  ...
  img.setAttribute('src', imgData.image.large);
  img.setAttribute('alt', imgData.title);
  const figcapt = document.createElement('figcaption');
  figcapt.innerText = imgData.caption;
  figure.appendChild(img);
  figure.appendChild(figcapt);
  const parag = document.createElement('p');
  parag.innerText = imgData.description;
  ...
  article.addEventListener('click', (event) => {
    modalImg.setAttribute('src', imgData.image.large);
    modalImg.setAttribute('alt', imgData.title);
    dialog.showModal();
  });
});

document.getElementById('pictures').appendChild(fragment);
